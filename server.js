...
app.post('/scan', (req, res) => {
  ...
  axios.get(url)
    .then(response => {
      ...
      const vulnerabilities = checkVulnerabilities(headers, response.data);
...
function checkVulnerabilities(headers, htmlContent) {
  let vulnerabilities = [];
  ...
  try {
    const signatures = [
      /<script.?>.?<\/script>/,    // malicious JavaScript
    ...
    for (const signature of signatures) {
      if (signature.test(htmlContent)) {
        vulnerabilities.push({
          name: 'Malicious Patterns',
          description: 'Potential malicious patterns detected in HTML content.',
          risk: 'High',
          evidence: 'Suspicious HTML patterns found.'
        });
        break; // Once a malicious pattern is found, no need to continue checking
      }
    }
  } catch (error) {
    console.error("An error occurred while scanning for malicious patterns:", error.message);
    vulnerabilities.push({
      name: 'Malicious Patterns Scanning Error',
      description: 'Error occurred while scanning for malicious patterns.',
      risk: 'Medium',
      evidence: 'Unable to scan for malicious patterns due to an error.'
    });
  }
